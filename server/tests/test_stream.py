...
from vex_agent.api.stream import format_sse_event
...
def test_proactive_message_carries_its_trigger():
    ...
    try:
        insert_message(
            ...
            student_id=student,
            role="assistant",
            message_text="Try changing a block.",
        ...
        tid = insert_agent_trigger_if_new(
            ...
            run_index=6,
            detail={"value": "6 identical reruns"},
        )
        ...
        with get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("DELETE FROM chat.messages WHERE student_id = %s", (student,))
                cur.execute(
                    "DELETE FROM event_logs.agent_triggers WHERE student_id = %s", (student,)
                )
