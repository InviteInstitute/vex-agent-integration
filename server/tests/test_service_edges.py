...
def test_detect_inactive_first_fire_uses_sentinel_run_index(monkeypatch):
    ...
    idle_ts = datetime.now(timezone.utc) - timedelta(seconds=400)
    monkeypatch.setattr(ts, "fetch_events_from_db",
                        lambda **k: [type("E", (), {"event_ts": idle_ts})()])
...
def test_detect_inactive_suppresses_realert_inside_window(monkeypatch):
    ...
    monkeypatch.setattr(ts, "fetch_events_from_db",
                        lambda **k: [type("E", (), {"event_ts": idle_ts})()])
    monkeypatch.setattr(ts, "latest_inactive_trigger",
                        lambda *a: (-1, now - timedelta(seconds=60)))  # fired 1 min ago
